import React from "react";
import LinkedinIcon from './socialCardAssets/linkedin.svg'; // Import the SVG as a component
import FacebookIcon from './socialCardAssets/facebook.svg'; // Import the SVG as a component

function SocialCard() {
  return (
    <div className="card-social">
      <div className="background"></div>
      <div className="card-logo fw-bold font-monospace">
        Tryck mig!
      </div>

      <a href="https://www.linkedin.com/company/enormt-se/?viewAsMember=true">
        <div className="box box1">
          <span className="icon">
            {/* Use the imported component here */}
            <img src={LinkedinIcon} className="svg" alt="LinkedIn" />          
            </span>
        </div>
      </a>

      <a href="https://www.instagram.com/enormt.se/">
        <div className="box box2">
          <span className="icon">
            {/* Include your inline SVG path here */}
            <svg viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg" className="svg">
              <path d="M 9.9980469 3 C 6.1390469 3 3 6.1419531 3 10.001953 L 3 20.001953 C 3 23.860953 6.1419531 27 10.001953 27 L 20.001953 27 C 23.860953 27 27 23.858047 27 19.998047 L 27 9.9980469 C 27 6.1390469 23.858047 3 19.998047 3 L 9.9980469 3 z M 22 7 C 22.552 7 23 7.448 23 8 C 23 8.552 22.552 9 22 9 C 21.448 9 21 8.552 21 8 C 21 7.448 21.448 7 22 7 z M 15 9 C 18.309 9 21 11.691 21 15 C 21 18.309 18.309 21 15 21 C 11.691 21 9 18.309 9 15 C 9 11.691 11.691 9 15 9 z M 15 11 A 4 4 0 0 0 11 15 A 4 4 0 0 0 15 19 A 4 4 0 0 0 19 15 A 4 4 0 0 0 15 11 z"></path>
            </svg>
          </span>
        </div>
      </a>

      <a href="https://www.facebook.com/profile.php?id=61550948814250">
        <div className="box box3">
          <span className="icon">
          <img src={FacebookIcon} className="svg" alt="Facebook" />
            {/* Use the imported component here */}
            {/* <FacebookIcon className="svg"/> */}
          </span>
        </div>
      </a>

      <div className="box box4"></div>
    </div>
  );
}

export default SocialCard;
